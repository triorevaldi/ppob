...
window.editIncome=id=>{let r=incomeData.find(x=>x.id===id);incomeEditing=id;$('item').value=r.item;$('qty').value=r.qty;$('amount').value=r.amount;$('date').value=dateInput(r.date);$('note').value=r.note;$('incomeForm').querySelector('button').textContent='Save changes'};
