...
function dateValue(s){let p=String(s||'').split('/');return p.length===3?new Date(+p[2],+p[1]-1,+p[0]).getTime():0}
...
window.editExpense=id=>{let r=expenseData.find(x=>x.id===id);expenseEditing=id;$('expenseItem').value=r.item;$('expenseQty').value=r.qty||1;$('expenseAmount').value=r.amount;$('expenseDate').value=r.date;$('expenseNote').value=r.note;$('expenseForm').querySelector('button').textContent='Save changes'};
...
function renderStock(){
  ...
  let rows=stockData.map(r=>{
    ...
    return `<tr><td>${r.date}</td><td>${esc(r.item)}</td><td>${qty}</td><td class="money">${fmt(cost)}</td><td>${stock.toLocaleString('id-ID')}</td><td>${stockPct.toLocaleString('id-ID',{maximumFractionDigits:2})}%</td><td class="money">${fmt(costPerUnit)}</td><td><button onclick="editStock('${r.id}')">Edit</button><button onclick="delStock('${r.id}')">Delete</button></td></tr>`
  }).join('');
...
$('stockForm').onsubmit=e=>{
  e.preventDefault();
  let r={date:$('stockDate').value.trim(),item:$('stockItem').value.trim(),qty:+$('stockQty').value,amount:+$('stockAmount').value,stock:+$('stockStock').value};
  if(stockEditing){
